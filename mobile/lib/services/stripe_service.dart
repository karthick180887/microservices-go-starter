...
class StripeService {
  ...
  static Future<void> redirectToCheckout(String sessionId) async {
    try {
      print(
          'StripeService: Redirecting to checkout with session ID: $sessionId');

      // Skip launching an external browser for mock sessions generated by the backend.
      if (sessionId.startsWith('mock_session')) {
        print(
            'StripeService: Mock session detected, skipping real checkout redirect.');
        return;
      }
